...
## 2026-07-22 局部共享组结构摘要

### 变更

- `compare_branches` 保留单一调用契约，在每个已返回的精确局部共享组中增加紧凑的
  `structural_summary`，只陈述支持关系类型和查询深度范围内实际观测到的连通事实。
- 不引入共同业务骨架、业务角色、重要性或异常分支推断；`pairwise_comparisons` 继续使用
  实体集合 Jaccard，`common_entity_ids` 和原证据缺口保持原义。
- 结构摘要明确采用精确共享实体组的诱导子图语义；关系实例存在于当前图且两端属于该组，
  不据此断言关系参与了每个成员分支的遍历路径。
- 拓扑查询只覆盖已返回局部共享组涉及的实体。查询失败或存在未解析分支时保留核心比较
  结果，以 `partial` 和 `STRUCTURAL_TOPOLOGY_UNAVAILABLE` 明确降级。

### 验证

- 新增完整关系键交集、单组件、多组件、稳定有界查询入口、零观测和服务降级测试。
- 已通过 `kg-statistic / project_stastic_month` 十任务、`max_depth=2` 两次正式查询验证：
  12 个局部共享组全部生成摘要，其中 1 个为多组件组；两次均响应 `ok` 且无 warning，
  摘要与 anchors 哈希一致。总耗时分别约 0.231/0.227 秒，拓扑查询约 0.067/0.071 秒，
  聚合后均返回 1258 条关系事实；相同基线的既有单次观测约 0.17 秒，新增结构字段约增加
  6.2 KB JSON 输出。以上为本机观测，不视为通用性能承诺。
...
- `query_layer/service.py`
  - 新增 graph-native `compare_branches` 原语，仅比较调用方明确传入的已确认结果分支，不自动发现或扩展业务范围。
  - 结果分支定义为：以一个已确认结果任务为入口、在指定深度内展开的关联图谱范围，不是 Git 分支。
  - 比较任务、数据集、字段、指标、指标定义、SQL 和结果字段，返回共同、差异、分支独有实体、局部共享组、两两相似度和结果字段派生差异。
  - 当 Neo4j 返回事务内存上限错误时，自动二分物理查询批次；请求的结果分支范围和遍历深度保持不变。

- `query_layer/cli.py` 和 `query_schemas.json`
  - 新增 `compare_branches` 请求契约和 `get_graph_native_capabilities` 能力发现入口。

- `QUERY_LAYER_USAGE.md`
  - 新增结果分支定义、调用示例、执行恢复机制和证据边界。

### 验证

- `knowledge-graph/tests/test_query_layer.py`：`14 passed`。
- 已覆盖全量范围先聚合后限制明细、构建版本校验、内存错误自动拆批、大范围物理分批和非内存错误不重试。
- 当前证据为自动化测试；尚未对真实 Neo4j 业务图执行结果分支比较验收。
